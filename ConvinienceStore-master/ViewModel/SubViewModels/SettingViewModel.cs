...
namespace ConvenienceStore.ViewModel.SubViewModels
{
    public class SettingViewModel : BaseViewModel
    {
        ...
        public void Save(Setting parameter)
        {
            ...
            {
                parameter.NameErrorMessage.Text = "Please enter your full name!!";
                isValid = false;
            }
            else
            {
                ...
                {
                    parameter.NameErrorMessage.Text = "Invalid name!!";
                    isValid = false;
                }
            }
            if (string.IsNullOrEmpty(parameter.emailTxtbox.textBox.Text))
            {
                parameter.EmailErrorMessage.Text = "Please enter email!!";
                isValid = false;
            }
            if (string.IsNullOrEmpty(parameter.phoneTxtbox.textBox.Text))
            {
                parameter.PhoneErrorMessage.Text = "Please enter phone number!!";
                isValid = false;
            }
            else
            {
                ...
                {
                    parameter.PhoneErrorMessage.Text = "Invalid phone number!!";
                    isValid = false;
                }
            }
            if (string.IsNullOrEmpty(parameter.addressTxtbox.textBox.Text))
            {
                parameter.AddressErrorMessage.Text = "Please enter Address!!";
                isValid = false;
            }
            ...
            foreach (var account in accounts)
            {
                ...
                {
                    parameter.PhoneErrorMessage.Text = "Phone number is registered!!";
                    isValid = false;
                    break;
                }
                if (parameter.emailTxtbox.textBox.Text == account.Email && parameter.emailTxtbox.textBox.Text != CurrentAccount.Email)
                {
                    parameter.EmailErrorMessage.Text = "The email was registered";
                    isValid = false;
                    break;
                }
            ...
            if (isValid)
            {
                ...
                MessageBoxCustom mb = new("Notification", "Edit successful", MessageType.Success, MessageButtons.OK);
                mb.ShowDialog();
